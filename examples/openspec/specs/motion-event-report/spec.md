...
设备 SHALL 在唤醒入网后,重新执行 SRP 注册(以 16 位小写十六进制 EUI64 为 host 名),待取得 BR(SRP server)地址后,单播 CoAP NON POST 到 BR 的 `/ack` 资源(URI、端口经 Kconfig 配置,须与 BR `mqtt_ot_bridge` 一致)上报事件。payload MUST 为手写 JSON `{"id":<eui64>,"reqid":<自生成>,"event":<事件类型>}`,MUST NOT 依赖 cJSON 托管组件。

#### Scenario: 运动事件上报
- **WHEN** 因运动唤醒且 SRP 注册取得 server 地址
- **THEN** 设备上报 `{"id":<eui64>,"reqid":<reqid>,"event":"motion"}` 到 BR `/ack`

#### Scenario: 心跳事件上报
- **WHEN** 因定时唤醒且 SRP 注册取得 server 地址
- **THEN** 设备上报 `event:"heartbeat"`
...
本能力 MUST NOT 要求改动 BR / `mqtt_ot_bridge`:上报走既有 `/ack` 资源,BR 原样透传 payload 到 MQTT `dev/response`,新增 `event` 字段随 payload 自动到达。

#### Scenario: event 字段透传到 MQTT
- **WHEN** 设备上报带 `event` 字段的 payload 到 BR `/ack`
- **THEN** MQTT `<prefix>/dev/response` 收到原样 payload(含 `event`),BR 无需改代码
