# border-router-wifi-provisioning Specification

## Purpose
定义 Thread Border Router 的开机 Wi-Fi 引导行为：在 NVS 凭据、编译期预设、SoftAP 网页配网三种来源之间的选择优先级，以及 SoftAP 配网结束后必须与 IDF Wi-Fi driver 状态对齐、干净初始化 STA 并联网、不得因重复 `esp_wifi_init` 而 abort。
## Requirements
