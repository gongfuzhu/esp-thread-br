...
4. **上报**：执行后单播 CoAP POST 到 BR 的 `MQTT_OT_BRIDGE_ACK_URI`(默认 `ack`)，
   payload 内带服务端下发的 reqid。
5. **组播抖动**：收到组播命令后随机延迟 0~500ms 再上报，避免响应风暴。

## 数据流
```
服务端 --MQTT/MQTTS(登陆)--> BR --CoAP--> H2 设备
  cmd/unicast/<eui64>  ->  CON 单播  -> 设备执行 -> 响应(同事务) -> dev/response
  cmd/multicast        ->  NON ff03::1 -> 各设备执行 -> 单播到 /ack -> dev/response
